...
func loadForestRoutes(e *forest.Forest, routes []*Route) {
	for _, r := range routes {
		switch r.Method {
		case "GET":
			e.GET(r.Path, forestHandler(r.Method, r.Path))
		case "POST":
			e.POST(r.Path, forestHandler(r.Method, r.Path))
		case "PATCH":
			e.PATCH(r.Path, forestHandler(r.Method, r.Path))
		case "PUT":
			e.PUT(r.Path, forestHandler(r.Method, r.Path))
		case "DELETE":
			e.DELETE(r.Path, forestHandler(r.Method, r.Path))
		}
	}
}

func forestHandler(method, path string) forest.HandlerFunc {
